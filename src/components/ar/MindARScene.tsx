...
export const MindARScene: FC<MindARSceneProps> = ({ 
  ...
  const compileTargetImage = useCallback(async (imageSrc: string): Promise<string> => {
        ...
        img.onload = async () => {
          try {
            ...
            await compiler.compileImageTargets([img], (progress: number) => {
              console.log('[MindAR] Compile progress:', Math.round(progress * 100) + '%');
            });
  ...
  // Initialize MindAR
  const initAR = useCallback(async (mindFileUrl: string) => {
    ...
    const loadAndInit = async () => {
      try {
        ...
        if (!window.MINDAR?.IMAGE?.MindARThree) {
          console.log('[MindAR] Loading MindAR...');
          await new Promise<void>((resolve, reject) => {
            const script = document.createElement('script');
      ...
      {!isInitialized && !error && (
        <div className="absolute inset-0 flex items-center justify-center bg-background/80 z-20">
          <div className="text-center">
            ...
            <p className="text-muted-foreground text-sm">Preparing AR experience...</p>
          </div>
        </div>
      )}
    </div>
